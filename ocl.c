...
_clState *initCl(unsigned int gpu, char *name, size_t nameSize, algorithm_t *algorithm)
{
  ...
  if (bufsize > 0) {
	...
	if(algorithm->type == ALGO_HMQ1725)
	{
		...
		// We need to assume the edge case - that the branch will go all one way,
		// resulting in it filling to the nonce size * GlobalThreadCount. Then,
		// allocate one more for the found index, and one for shit happens.
		
		clState->Branch1Nonces = clCreateBuffer(clState->context, CL_MEM_READ_WRITE, sizeof(cl_uint) * (clState->GlobalThreadCount + 2), NULL, &status);
		if(status != CL_SUCCESS)
		{
			applog(LOG_ERR, "Error %d while creating Branch1Nonces buffer.", status);
			return(NULL);
		}
		
		clState->Branch2Nonces = clCreateBuffer(clState->context, CL_MEM_READ_WRITE, sizeof(cl_uint) * (clState->GlobalThreadCount + 2), NULL, &status);
		if(status != CL_SUCCESS)
		{
			applog(LOG_ERR, "Error %d while creating Branch1Nonces buffer.", status);
			return(NULL);
		...
		clState->Branch3Nonces = clCreateBuffer(clState->context, CL_MEM_READ_WRITE, sizeof(cl_uint) * (clState->GlobalThreadCount + 2), NULL, &status);
		if(status != CL_SUCCESS)
		{
			applog(LOG_ERR, "Error %d while creating Branch3Nonces buffer.", status);
			return(NULL);
		}
		
		clState->Branch4Nonces = clCreateBuffer(clState->context, CL_MEM_READ_WRITE, sizeof(cl_uint) * (clState->GlobalThreadCount + 2), NULL, &status);
		if(status != CL_SUCCESS)
		{
			applog(LOG_ERR, "Error %d while creating Branch4Nonces buffer.", status);
			return(NULL);
		}
		
		clState->Branch5Nonces = clCreateBuffer(clState->context, CL_MEM_READ_WRITE, sizeof(cl_uint) * (clState->GlobalThreadCount + 2), NULL, &status);
		if(status != CL_SUCCESS)
		{
			applog(LOG_ERR, "Error %d while creating Branch5Nonces buffer.", status);
			return(NULL);
		}
		
		clState->Branch6Nonces = clCreateBuffer(clState->context, CL_MEM_READ_WRITE, sizeof(cl_uint) * (clState->GlobalThreadCount + 2), NULL, &status);
		if(status != CL_SUCCESS)
		{
			applog(LOG_ERR, "Error %d while creating Branch6Nonces buffer.", status);
			return(NULL);
		}
		
		clState->Branch7Nonces = clCreateBuffer(clState->context, CL_MEM_READ_WRITE, sizeof(cl_uint) * (clState->GlobalThreadCount + 2), NULL, &status);
		if(status != CL_SUCCESS)
		{
			applog(LOG_ERR, "Error %d while creating Branch7Nonces buffer.", status);
			return(NULL);
		}
		
		clState->Branch8Nonces = clCreateBuffer(clState->context, CL_MEM_READ_WRITE, sizeof(cl_uint) * (clState->GlobalThreadCount + 2), NULL, &status);
		if(status != CL_SUCCESS)
		{
			applog(LOG_ERR, "Error %d while creating Branch8Nonces buffer.", status);
			return(NULL);
		}
		
		clState->Branch9Nonces = clCreateBuffer(clState->context, CL_MEM_READ_WRITE, sizeof(cl_uint) * (clState->GlobalThreadCount + 2), NULL, &status);
		if(status != CL_SUCCESS)
		{
			applog(LOG_ERR, "Error %d while creating Branch9Nonces buffer.", status);
			return(NULL);
		}
		
		clState->Branch10Nonces = clCreateBuffer(clState->context, CL_MEM_READ_WRITE, sizeof(cl_uint) * (clState->GlobalThreadCount + 2), NULL, &status);
		if(status != CL_SUCCESS)
		{
			applog(LOG_ERR, "Error %d while creating Branch10Nonces buffer.", status);
			return(NULL);
		}
		
		clState->Branch11Nonces = clCreateBuffer(clState->context, CL_MEM_READ_WRITE, sizeof(cl_uint) * (clState->GlobalThreadCount + 2), NULL, &status);
		if(status != CL_SUCCESS)
		{
			applog(LOG_ERR, "Error %d while creating Branch11Nonces buffer.", status);
			return(NULL);
		}
		
		clState->Branch12Nonces = clCreateBuffer(clState->context, CL_MEM_READ_WRITE, sizeof(cl_uint) * (clState->GlobalThreadCount + 2), NULL, &status);
		if(status != CL_SUCCESS)
		{
			applog(LOG_ERR, "Error %d while creating Branch12Nonces buffer.", status);
			return(NULL);
		}
		
		clState->Branch13Nonces = clCreateBuffer(clState->context, CL_MEM_READ_WRITE, sizeof(cl_uint) * (clState->GlobalThreadCount + 2), NULL, &status);
		if(status != CL_SUCCESS)
		{
			applog(LOG_ERR, "Error %d while creating Branch13Nonces buffer.", status);
			return(NULL);
		}
		
		clState->Branch14Nonces = clCreateBuffer(clState->context, CL_MEM_READ_WRITE, sizeof(cl_uint) * (clState->GlobalThreadCount + 2), NULL, &status);
		if(status != CL_SUCCESS)
		{
			applog(LOG_ERR, "Error %d while creating Branch14Nonces buffer.", status);
			return(NULL);
		}
		
		clState->Branch15Nonces = clCreateBuffer(clState->context, CL_MEM_READ_WRITE, sizeof(cl_uint) * (clState->GlobalThreadCount + 2), NULL, &status);
		if(status != CL_SUCCESS)
		{
			applog(LOG_ERR, "Error %d while creating Branch15Nonces buffer.", status);
			return(NULL);
		}
		
		clState->Branch16Nonces = clCreateBuffer(clState->context, CL_MEM_READ_WRITE, sizeof(cl_uint) * (clState->GlobalThreadCount + 2), NULL, &status);
		if(status != CL_SUCCESS)
		{
			applog(LOG_ERR, "Error %d while creating Branch16Nonces buffer.", status);
			return(NULL);
		}
	}
