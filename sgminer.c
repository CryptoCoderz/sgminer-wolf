...
#ifdef HAVE_LIBCURL
static char *opt_gbt_address;
static char *opt_gbt_sig;
#endif
...
struct opt_table opt_config_table[] = {
 ...
 OPT_WITH_ARG("--log-file|-L",
  ...
  OPT_WITH_ARG("--difficulty-multiplier",
    ...
    #ifdef HAVE_LIBCURL
       OPT_WITH_ARG("--gbt-address",
                    opt_set_charp, NULL, &opt_gbt_address,
                    "Set groestlcoin target address when solo mining to groestlcoind (mandatory)"),
       OPT_WITH_ARG("--gbt-sig",
                    opt_set_charp, NULL, &opt_gbt_sig,
...
static double diff_from_target(algorithm_t algo, void *target);
...
static bool gbt_solo_decode(struct pool *pool, json_t *res_val)
{
	...
	hex2bin(hash_swap, target, 32);
	swab256(pool->gbt_target, hash_swap);
	pool->sdiff = diff_from_target(pool->algorithm, pool->gbt_target);
	...
	*u32 = htole32(now.tv_usec);
	...
	if (opt_gbt_sig) {
		...
		pool->scriptsig_base[ofs++] = len;
		cg_memcpy(pool->scriptsig_base + ofs, opt_gbt_sig, len);
		ofs += len - 1;
	}
...
static bool submit_upstream_work(struct work *work, CURL *curl, char *curl_err_str, bool resubmit)
{
  ...
  if (work->gbt) {
    ...
    s = (char *)malloc(sizeof(char) * (1024 + strlen(gbt_block)));
    sprintf(s, "{\"id\": 0, \"method\": \"submitblock\", \"block\": \"%s\" \"params\": [\"", gbt_block);
...
static double diff_from_target(algorithm_t algo, void *target)
{
	...
	d64 = algo.diff_multiplier2 * truediffone;
...
static bool setup_gbt_solo(CURL *curl, struct pool *pool)
{
	...
	if (!opt_gbt_address) {
		applog(LOG_ERR, "No address specified, unable to mine solo on %s",
		       pool->rpc_url);
		goto out;
	}
	...
	if (!json_is_true(valid_val)) {
		applog(LOG_ERR, "Address %s is NOT valid", opt_gbt_address);
		goto out;
	}
	applog(LOG_NOTICE, "Solo mining to valid address: %s", opt_gbt_address);
	ret = true;
